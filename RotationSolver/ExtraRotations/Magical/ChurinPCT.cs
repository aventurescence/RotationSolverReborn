...
[Rotation("Churin PCT", CombatType.PvE, GameVersion = "7.31")]
[SourceCode(Path = "main/ExtraRotations/Magical/ChurinPCT.cs")]

public sealed class ChurinPCT : PictomancerRotation
{
    #region Properties

    public static bool HasNoMotif => !CreatureMotifDrawn && !LandscapeMotifDrawn && !WeaponMotifDrawn;
    #endregion
    #region Config Options
    [RotationConfig(CombatType.PvE, Name = "Use HolyInWhite or CometInBlack while moving")]
    private bool HolyCometMoving { get; set; } = true;

    [RotationConfig(CombatType.PvE, Name = "Paint overcap protection.")]
    private bool UseCapCometHoly { get; set; } = true;

    [RotationConfig(CombatType.PvE, Name = "Use the paint overcap protection (will still use comet while moving if the setup is on)")]
    private bool UseCapCometOnly { get; set; } = false;
    ...
    {
        if (remainTime < RainbowDripPvE.Info.CastTime + 0.5f)
        {
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? act)
    {
        ...
        // Bursts
        var adjustCombatTimeForOpener = Player.Level < 92 ? 2 : 5;
        if (InCombat && CombatElapsedLessGCD(2) && IsLastGCD(ActionID.RainbowDripPvE) && PomMusePvE.CanUse(out act))
        {
            return true;
        }
        if (InCombat && CombatElapsedLessGCD(2) && IsLastAbility(ActionID.PomMusePvE) && StrikingMusePvE.CanUse(out act))
        {
            return true;
        }
        ...
        if (HasStarryMuse)
        {
            if (CombatElapsedLess(60) && !HasSubtractivePalette && WingedMusePvE.CanUse(out act))
            {
                return true;
            }
        ...
        return ClawedMusePvE.CanUse(out act, usedUp: true) ||
    ...
    protected override bool GeneralGCD(out IAction? act)
    {
        ...
        if (HasStarryMuse && HammerStampPvE.CanUse(out act))
        {
            return true;
        }

        if (HasSubtractivePalette)
        {
            return TryUseSubtractiveInks(out act);
        }
    ...
    private bool TryUseSubtractiveInks(out IAction? act)
    {
        act = null;
        if (!HasSubtractivePalette) return false;
        return BlizzardInCyanPvE.CanUse(out act)
               || ThunderInMagentaPvE.CanUse(out act)
               || StoneInYellowPvE.CanUse(out act);
    }
    #endregion
}