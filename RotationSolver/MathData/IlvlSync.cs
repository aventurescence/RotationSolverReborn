    ...
    public static unsafe (uint?, IlvlSyncType) GetCurrentIlvlSync() {
        ...
        if (icDirector != null) {
            if (icDirector->InstanceContentType != InstanceContentType.BeginnerTraining && (icDirector->ContentDirector.Director.ContentFlags & 1) == 0) {
                ...
                {
                    PluginLog.Debug($"Using min ilvl {*(ushort*)(icd + 1328)}");
                ...
                {
                    PluginLog.Debug($"Using duty ilvl sync {*(ushort*)(icd + 1330)}");
        ...
        if (pcDirector != null)
        {
            ...
            {
                PluginLog.Debug($"Using public content ilvl sync {*(ushort*)(pcd + 1330)}");
        ...
        if (UIState.Instance()->PlayerState.IsLevelSynced)
        {
            ...
            PluginLog.Debug($"Using level based ilvl {ilvl}");
